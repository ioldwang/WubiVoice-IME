...
struct SquirrelApp {
  ...
  static let logDir = FileManager.default.temporaryDirectory.appending(component: "wubivoice", directoryHint: .isDirectory)
  ...
  static func main() {
    ...
    let handled = autoreleasepool {
      ...
      if args.count > 1 {
        ...
        case "--build":
          ...
          builderTraits.setCString("ai.hojo.WubiVoice.builder", to: \.app_name)
    ...
    autoreleasepool {
      ...
      if NSApp.squirrelAppDelegate.problematicLaunchDetected() {
        ...
        let args = ["Problematic launch detected! WubiVoice may be suffering a crash due to improper configuration. Revert previous modifications to see if the problem recurs."]
        let task = Process()
        ...
        print("WubiVoice reporting!")
      }

      // finally run everything
      app.run()
      print("WubiVoice is quitting...")
      rimeAPI.finalize()
    }
    return
  }
...
Supported arguments:
Perform actions:
  --quit                     quit all WubiVoice processes
  --reload                   deploy
  --sync                     sync user data
