...
final class SquirrelInstaller {
  enum InputMode: String, CaseIterable {
    ...
    case hans = "ai.hojo.WubiVoice.Hans"
    case hant = "ai.hojo.WubiVoice.Hant"
  }
  ...
  func register() {
    ...
    if !enabledInputModes.isEmpty {
      print("User already registered WubiVoice method(s): \(enabledInputModes.map { $0.rawValue })")
      // Already registered.
      return
    }
  ...
  func enable(modes: [InputMode] = []) {
    ...
    if !enabledInputModes.isEmpty && modes.isEmpty {
      print("User already enabled WubiVoice method(s): \(enabledInputModes.map { $0.rawValue })")
